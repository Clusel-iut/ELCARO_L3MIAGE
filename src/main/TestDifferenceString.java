...
public class TestDifferenceString {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nDifference des noms supérieur à 6 - prénom inférieur à 8");
		...
		System.out.println("\nDifference des noms qui commence par P ou C - prénom qui commence par P");
		Relation s2 = new Difference(r, new Predicat() {
			@Override
			public boolean eval(Tuple tuple) {
				return ((StringBuff) tuple.getValue(0)).sb.toString().startsWith("P")
						|| ((StringBuff) tuple.getValue(0)).sb.toString().startsWith("C");
			}
		}, new Predicat() {
			@Override
			public boolean eval(Tuple tuple) {
